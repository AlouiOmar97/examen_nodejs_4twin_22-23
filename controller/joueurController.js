...
async function addPartie(req, res) {
  try {
    const Partie = new partie({
      nom: req.body.nom,
      joueur1: req.body.id1,
      joueur2: req.body.id2,
      etat: "en cours",
      gagnant: "",
    });
    const j1u = await joueur.findByIdAndUpdate(req.body.id1, {
      sante: 100,
    });
    const j2u = await joueur.findByIdAndUpdate(req.body.id2, {
      sante: 100,
    });
    await Partie.save();
    res.status(200).send("add partie success");
  } catch (err) {
    console.log({ error: error.toString() });
  }
}
